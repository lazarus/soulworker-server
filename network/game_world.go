...
func (gameWorld *GameWorld) process(channel *Connection, packetID uint16, buffer *bytes.Buffer) int {
	if buffer.Len() == 0 {
		return 0
	}
	...
	if packetID == 0x0321 {
		...
		var ClientEnterGameServerReq struct {
			AccountId   uint32
			CharacterId uint32
			Unknown     uint64
			Unknown2    byte
		...
		_ = binary.Read(buffer, binary.LittleEndian, &ClientEnterGameServerReq.CharacterId)
		_ = binary.Read(buffer, binary.LittleEndian, &ClientEnterGameServerReq.Unknown)
		_ = binary.Read(buffer, binary.LittleEndian, &ClientEnterGameServerReq.Unknown2)
		_ = binary.Read(buffer, binary.LittleEndian, &ClientEnterGameServerReq.SessionKey)
		...
		channel.writeQueue <- global.Packet{ID: 0x0403, Data: serverWorldCurDate.Build()}
		...
		channel.writeQueue <- global.Packet{ID: 0x0404, Data: serverWorldVersion.Build()}

		serverEnterGameServerRes := ServerEnterGameServerRes{
			Result:  1,
		...
		channel.writeQueue <- global.Packet{ID: 0x0322, Data: serverEnterGameServerRes.Build()}
	...
	} else if packetID == 0x0331 { // ClientCharacterInfoRequest

		var characterId uint32
		_ = binary.Read(buffer, binary.LittleEndian, &characterId)
		_ = buffer.Next(1)


		charInfo, err := database.FetchUserCharacterByCharacterId(characterId)
		if err != nil {
			log.Fatal(err)
		}
		...
		serverCharacterInfoRes := ServerCharacterInfoRes{
		...
		channel.writeQueue <- global.Packet{ID: 0x0332, Data: serverCharacterInfoRes.Build()}
	} else if packetID == 0x0105 {
		packet := []byte{
